...
FHIR.oauth2.ready().then(function(client) {
  ...
  client.request("Observation?" + query, {
    ...
    function(ob) {
      ...
      var systolicbps = getBloodPressureValue(byCodes('55284-4'), '8480-6');
      var diastolicbps = getBloodPressureValue(byCodes('55284-4'), '8462-4');
      ...
      var pair = item => {return {"value": getQuantityValueAndUnit(item), "time": new Date(item.effectiveDateTime)}};

      // map vitals to time value pairs
      heights = heights.map(pair);
      weights = weights.map(pair);
      hdl = hdl.map(pair);
      ldl = ldl.map(pair);
      systolicbps = systolicbps.map(pair);
      diastolicbps = diastolicbps.map(pair);
